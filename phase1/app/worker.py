...
async def run(payload: dict) -> dict:
    ...
    standardized_pack = [page.model_dump() for page in tagged_pages]
...
# ═══════════════════════════════════════════════════════════════════
#  TASK #p1.3 — EDITABILITY TAGGING (asyncio.gather + batching)
# ═══════════════════════════════════════════════════════════════════
...
- "non-editable": Absolutely locked — copyright notices, character faces, logos.

Rules for classification:
1. Text containing ONLY protected names → "non-editable"
2. Copyright notices, ISBN, publisher info → "non-editable"
3. Text near character faces or overlapping character art → "semi-editable"
4. SFX / onomatopoeia in images → depends on sfx_handling setting
5. Regular story text → "editable"
6. Background images without characters → "editable"
7. Images containing main characters → "semi-editable" or "non-editable"

Return a JSON array where each element has:
{"block_index": <int>, "block_type": "text"|"image"|"ocr", "editability_tag": "editable"|"semi-editable"|"non-editable"}
...
async def _tag_page_editability(
    ...
    constraints_summary = (
        ...
        f"No retouching: {metadata.no_retouching}\n"
        f"Lock character color: {metadata.lock_character_color}\n"
        f"SFX handling: {metadata.sfx_handling}\n"
